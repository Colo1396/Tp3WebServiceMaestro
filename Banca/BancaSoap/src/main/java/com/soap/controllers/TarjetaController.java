...
public class TarjetaController extends Conexion {
	...
	public List<Tarjeta> findAll() throws SQLException {
		try {
			...
			while (rs.next()) {
				...
				Tarjeta tarjeta = new Tarjeta(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getDouble(4),
	...
	// ------------------------------------------------------------------------------------
	public double traerLimiteDisponible(int dni, String nroTarjeta) throws SQLException {
		try {
